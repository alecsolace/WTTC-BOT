...
class Migration20240303184811 extends Migration {
  ...
  async up() {
    ...
    this.addSql('create table `ship` (`id` int unsigned not null auto_increment primary key, `created_at` datetime not null, `updated_at` datetime not null, `manufacturer_id` int unsigned not null, `model` varchar(255) not null, `owner_id` int unsigned not null, `name` varchar(255) null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `ship` add index `ship_manufacturer_id_index`(`manufacturer_id`);');
    this.addSql('alter table `ship` add index `ship_owner_id_index`(`owner_id`);');
    ...
    this.addSql('alter table `ship` add constraint `ship_owner_id_foreign` foreign key (`owner_id`) references `member` (`id`) on update cascade;');
  }

  async down() {
    this.addSql('alter table `ship` drop foreign key `ship_manufacturer_id_foreign`;');

    this.addSql('alter table `ship` drop foreign key `ship_owner_id_foreign`;');
...
exports.Migration20240303184811 = Migration20240303184811;
